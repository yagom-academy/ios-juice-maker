...
class JuiceMakerUnitTests: XCTestCase {
    ...
    func test_makeJuice_딸기쥬스_재조시_딸기재고소진_에러가발생한다() {
        // given
        let inputJuice: Drinkable = Juice.strawberryJuice
        
        // then
        XCTAssertThrowsError(try juiceMaker?.makeDrink(inputJuice), "makeJuiice 재고 Error") { error in
    ...
    func test_makeJuice_딸기가_16개일_때_딸기쥬스를_제조하고_나면_재고가_0개가_된다() {
        ...
        // when
        _ = try? self.juiceMaker?.makeDrink(inputJuice)
    ...
    func test_makeJuice_딸바쥬스를_제조하고_나면_딸기재고_0개_바나나재고_9개가_된다() {
        ...
        let inputJuice: Drinkable = Juice.strawberryBananaJuice
    ...
    func test_makeJuice_망고재고가_부족할_때_망고키위쥬스를_만들면_에러가_발생한다() {
        // given
        let inputJuice: Drinkable = Juice.mangoKiwiJuice
        try? self.fruitStore?.decrease(fruit: .mango, to: 10)
        
        // when
        XCTAssertThrowsError(try juiceMaker?.makeDrink(inputJuice), "makeJuice 재고 Error") { error in
            // then
            XCTAssertEqual(error as? JuiceMakerError, JuiceMakerError.outOfStock)
        }
    }
